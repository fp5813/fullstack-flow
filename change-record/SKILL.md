---
name: fullstack-flow/change-record
description: 生成修改记录文件（fullstack-flow 子技能），包含前后代码对比和回滚方案。格式参考 TEMPLATE.md，输出到 docs/修改记录/ 目录。
allowed-tools:
  - Read
  - Write(docs/**/*.md)
  - Edit(docs/**/*.md)
---

# 修改记录生成

> 根据当前代码变更生成修改记录文件，包含前后代码对比和回滚方案（不使用 git）。

## 快速概览

```
Step 1 收集修改信息 → Step 2 提取前后代码对比 → Step 3 生成回滚方案
  → Step 4 写入文件 → Step 5 同步知识库 → 完成
```

**核心**: 前后代码对比（各 3 行上下文）+ 手动回滚方案（禁止 git）

## 执行流程

### Step 1: 收集修改信息

根据对话上下文或用户输入，列出所有修改的文件，分隔每个文件内的独立修改点。

### Step 2: 提取前后代码

每个修改点独立记录，包含完整上下文（前后各 3 行）。

### Step 3: 生成回滚方案

每个修改点对应一个回滚步骤，**不使用 git 命令**（手动替换代码）。

### Step 4: 写入文件

路径：`docs/修改记录/YYYY-MM-DD-功能简述.md`

### Step 5: 同步知识库（可选）

如项目配置了知识库同步工具，运行对应的同步命令。路径参考 `CODEBUDDY.md` 中配置。

## 模板格式

详细模板和编写规范参考主技能的参考文档：

```
${CODEBUDDY_SKILL_DIR}/../references/change-record-detailed.md
```

## 质量检查

- [ ] 文件路径正确：`docs/修改记录/YYYY-MM-DD-功能简述.md`
- [ ] 每个修改点都有修改前 / 修改后代码对比
- [ ] 回滚方案不使用 git 命令
- [ ] 新增文件标注了创建，删除文件标注了恢复方案
