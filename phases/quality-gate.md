...
1. Read `.codebuddy/workflow/state.yaml`，验证 `phase.current == "quality-gate"`
...
2. Phase 出口：
   ...
   - `progress.phases_completed.append("quality-gate")`
   - 通过时：`phase.current = "spec"`, `phase.status = "pending"`
   - 不通过（<100%）时：`phase.current = "probe"`, `phase.status = "pending"`, `progress.phases_blocked.append("probe: 门控未通过")`, `metrics_snapshot.gate_retries += 1`
   - `metrics_snapshot.phase_durations[quality-gate] = 耗时分钟数`
