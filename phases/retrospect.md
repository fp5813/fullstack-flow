...
1. Read `.codebuddy/workflow/state.yaml`，验证 `phase.current == "retrospect"`
...
3. Phase 出口：
   ...
   - `progress.phases_completed.append("retrospect")`
   - `phase.current = "rule-sync"`, `phase.status = "pending"`
   - `metrics_snapshot.phase_durations[retrospect] = 耗时分钟数`
