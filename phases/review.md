...
1. Read `.codebuddy/workflow/state.yaml`，验证 `phase.current == "review"`
...
| 数据库验证 | `mysql-{子项目名}.execute_query` | 检查数据正确性 | 与预期一致 |
...
- Agent BE 只审后端文件（探路报告记录的后端路径），前置加载 `java-standards` 子技能
- 标注更新日期
- 审核记录必输出
- API 文档缺失必须补充
- 涉及新增模块时同步更新接口文档目录
...
3. Phase 出口：
   ...
   - `progress.phases_completed.append("review")`
   - 通过时：`phase.current = "record"`, `phase.status = "pending"`
   - 有代码问题时：`phase.current = "code"`, `phase.status = "pending"`, `progress.phases_blocked.append("code: 审核发现代码问题")`, `metrics_snapshot.gate_retries += 1`
   - `metrics_snapshot.phase_durations[review] = 耗时分钟数`
