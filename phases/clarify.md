...
| 信息类型 | 验证方式 | 示例 |
|---------|---------|------|
| 模块/页面路径 | `Glob` 匹配 `src/{project-path}/{模块}/` | 用户说"订单管理页面" → Glob 确认 `controller/product/OrderController.java` 存在 |
| 方法名/API 路径 | `Grep` 或 `codegraph` 搜索 Controller | 用户说"报错接口是 /system/xxx/list" → Grep Controller 层确认 |
| 表名 | `mysql-{子项目名}` 查询表结构，或 Grep Entity 注解 | 用户说"表名是 product_order" → 检查对应的 Entity 或数据库表 |
