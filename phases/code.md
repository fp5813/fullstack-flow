...
1. Read `.codebuddy/workflow/state.yaml`，验证 `phase.current == "code"`
...
2. Phase 出口：
   ...
   - `progress.phases_completed.append("code")`
   - `phase.current = "review"`, `phase.status = "pending"`
   - `metrics_snapshot.phase_durations[code] = 耗时分钟数`
